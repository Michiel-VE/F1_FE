...
export class DriverService {
  ...
  private isDataReloadNecessary(lastReloadTimestamp: number): boolean {
    ...
    const isReloadNecessary = lastReloadDate.getTime() < requiredReloadTime.getTime();

    return isReloadNecessary;
  }
  ...
  getData<T>(key: string, year: string = this.getCurrentYear()): Observable<T> {
    return this.cacheService.getItem<T>(key).pipe(
      switchMap((entry) => {
        const cacheMissing = !entry;
        const needsReload = this.isDataReloadNecessary(entry?.timestamp || 0);
        const cacheExpiredOrMissing = cacheMissing || needsReload;
